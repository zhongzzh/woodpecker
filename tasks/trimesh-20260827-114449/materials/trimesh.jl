...
@testset "trimesh 补充" begin
    ...
    @testset "矩阵输入形式" begin
        h2_mat = trimesh(connectivity, Xmat, Ymat)
        @test h2_mat !== nothing

        h3_mat = trimesh(connectivity, Xmat, Ymat, Zmat)
        @test h3_mat !== nothing
    end

    @testset "facecolor 颜色名称补充" begin
        for color in ["red", "green", "g", "blue", "b"]
            h_fc_name = trimesh(connectivity, x, y, z; facecolor=color)
            @test h_fc_name !== nothing
        end
    end

    @testset "facecolor 十六进制形式补充" begin
        for color in ["#FF8800", "#ff8800"]
            h_fc_hex = trimesh(connectivity, x, y, z; facecolor=color)
            @test h_fc_hex !== nothing
        end
    end

    @testset "edgecolor 颜色名称补充" begin
        for color in ["red", "green", "g", "blue", "b"]
            h_ec_name = trimesh(connectivity, x, y, z; edgecolor=color)
            @test h_ec_name !== nothing
        end
    end

    @testset "edgecolor 十六进制形式补充" begin
        for color in ["#FF8800", "#ff8800"]
            h_ec_hex = trimesh(connectivity, x, y, z; edgecolor=color)
