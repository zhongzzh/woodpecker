...
writeVideo([v](/Doc/TyImageProcessing/DataImportandAnalysis/DataImportandExport/StandardFileFormats/AudioandVideoData/ReadorWriteVideo/writeVideo.html#fa637a1c),[frame](/Doc/TyImageProcessing/DataImportandAnalysis/DataImportandExport/StandardFileFormats/AudioandVideoData/ReadorWriteVideo/writeVideo.html#frame)) 写入通常由 getframe 函数返回的一个或多个影片帧。
...
<div id="frame" class="jump-target"></div>
<div class="details-box">
<details open>
<summary>frame — 帧数据<div>Frame_struct 对象 | Frame_struct 对象的数组</div></summary>
</details>
<div class="details-content">

帧数据，指定为表示单个帧的 Frame_struct 对象，或表示多个帧的 Frame_struct 对象数组。每个帧都包含两个字段：`cdata` 和 `colormap`。`frame` 通常由 getframe 函数返回。

writeVideo 使用 `cdata` 字段构造图像帧，忽略 `colormap` 字段。
